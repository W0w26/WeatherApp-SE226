...
root.geometry('500x500')  # sığmayan il isimleri için büyütüldü
root.resizable(0, 0)
...
def get_weather():
    ...
    try:
        ...
        wind_speed = str(response['wind']['speed'])  # Eklenen satır
        ...
        if search_method.get() == 1:
            ...
            wind_speed_label.config(text="Wind Speed: " + wind_speed + " m/s", font=small_font, bg=output_color)  # Eklenen satır
        elif search_method.get() == 2:
            city_info_label.config(text=city_name + "{" + city_lat + ", " + city_lon + ")", font=large_font,
                                   bg=output_color)
            weather_label.config(text="Weather: " + main_weather + ", " + description, font=small_font,
            ...
            temp_max_label.config(text="Max Temperature: " + temperature_max + " F", font=small_font,
                                  bg=output_color)
            humidity_label.config(text="Humidity: " + humidity, font=small_font, bg=output_color)
            wind_speed_label.config(text="Wind Speed: " + wind_speed + " mph", font=small_font, bg=output_color)  # Eklenen satır
    except KeyError:
        messagebox.showerror("Error", "Invalid response received from the weather service.")
    except Exception as e:
        messagebox.showerror("Error", str(e))
...
output_frame = tkinter.LabelFrame(sky_frame, bg=output_color, width=400, height=225)  #sığmayan il isimleri için wildh arttırıldı
