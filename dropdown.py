...
def search():
    ...
    try:
        ...
        messagebox.showerror("Error", "Invalid response received from the weather service.")
...
def get_weather():
    ...
    try:
        ...
        if search_method.get() == 1:
            ...
            wind_speed_label.config(text="Wind Speed: " + wind_speed + " m/s", font=small_font, bg=output_color)
        elif search_method.get() == 2:
            city_info_label.config(text=city_name + "{" + city_lat + ", " + city_lon + ")", font=large_font,
                                   bg=output_color)
            weather_label.config(text="Weather: " + main_weather + ", " + description, font=small_font,
            ...
            temp_min_label.config(text="Min Temperature: " + temperature_min + " F", font=small_font,
                                  bg=output_color)
            temp_max_label.config(text="Max Temperature: " + temperature_max + " F", font=small_font,
                                  bg=output_color)
            humidity_label.config(text="Humidity: " + humidity, font=small_font, bg=output_color)
            wind_speed_label.config(text="Wind Speed: " + wind_speed + " mph", font=small_font, bg=output_color)
...
temp_min_label = tkinter.Label(output_frame, bg=output_color, text='Please enter the city name',
                               font=('SimSun', 15))
temp_max_label = tkinter.Label(output_frame, bg=output_color, text='below in the box',
                               font=('SimSun', 15))
...
dropdown_options = tkinter.StringVar(value='yesterday\ntoday\ntomorrow')

dropdown_label = tkinter.Label(input_frame, text='Select the day', font=small_font, bg=input_color)
dropdown_menu = tkinter.OptionMenu(input_frame, dropdown_options, *dropdown_options.get().split("\n"))
...
dropdown_label.grid(row=2, column=0, pady=2, columnspan=2)
dropdown_menu.grid(row=3, column=0, pady=2, columnspan=2)
